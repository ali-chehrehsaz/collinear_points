...
#     with pytest.raises(TypeError):
#         assert get_lines('some_string')
#         assert get_lines('{0: 0, 1: 1, 2: 2}')
#         assert get_lines('tuple((0, 0), (1, 1), (2, 2))')
#         assert get_lines('set((0, 0), (1, 1), (2, 2)))')


# def test_raise_exception_when_input_data_xy_points_is_not_expected_tuple_of_2():
#     with pytest.raises(TypeError):
#         assert get_lines(['1, 2', '4, 4'])
#         assert get_lines([[1, 2], [4, 4]])
#         # assert collinear_pnts([(1, 2, 3), (4, 4, 4)])
#
#
# def test_raise_exception_when_xy_tuple_is_less_or_more_than_two_numeric():
#     with pytest.raises(IndexError):
#         points = [(1, 1), (2, 2), (3, 3, 99999)]
#         assert get_lines(points)
#
#
# def test_raise_exception_when_xy_points_not_tuple():
#     with pytest.raises(TypeError):
#         assert get_lines(['1, 2', '4, 4'])
#         assert get_lines([[1, 2], [4, 4]])
#         # assert collinear_pnts([(1, 2, 3), (4, 4, 4)])
#
#
# def test_raise_exception_when_xy_points_not_tuple_of_2_numeric():
#     with pytest.raises(ValueError):
#         assert get_lines([(3, 3, 3)])
#
#
